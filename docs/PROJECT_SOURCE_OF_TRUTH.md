...
| VLA | contenedores persistentes detenidos, `restart=no`, cero publicadores. E6.0 NO_BOX está retirado. E6.1A `20260904T103516_E6.1A` congeló `episode_000040/task 0`; E6.1B `20260904T114452_E6.1B` implementó offline ENTRY/recovery no ejecutables, gate exacto 20D y campaña de cinco shadow con evidencia RGB+estado. La mesa disponible de `0,838 × 0,84 × 0,77 m` pasa la regresión dimensional para shadow estacionario una vez congelado el fixture; su OBB conservador alerta, pero no demuestra contacto ni la descarta. | **E6.1=40 %; E6.1A/B PASS OFFLINE; FALTAN FIXTURE/ENTRY VIVOS Y 5 SHADOW, SIN MOVIMIENTO AUTORIZADO** |
...
E6.1B quedó implementado offline en `20260904T114452_E6.1B`: fijó los 20 ejes
...
`0,838 × 0,84 m` y `0,77 m` de altura, queda como candidata para shadow
estacionario después de medir su espesor, congelar pose/foto/hashes y situar el
robot en la ENTRY por un procedimiento separado. Con espesores hipotéticos de
...
El plan ponderado queda en `45,8 %`, las cuatro tareas físicas del checkpoint
siguen en `0/4` y E6.1 queda al `40 %`: E6.1A y el paquete offline E6.1B están
cerrados; faltan manifiesto físico, ENTRY fresca y las cinco sesiones shadow.
...
|---|---|---|
| 2026-09-04 | E6.1B ENTRY/recovery y cinco shadow fail-closed | run offline `20260904T114452_E6.1B`: contrato exacto `episode_000040/frame 0/task 0`, previews ENTRY/recovery sin ejecutor, gate de fixture+estado fresco 20D y campaña de cinco sesiones task 0/P14 con evidencia RGB+estado sincronizada. La regresión de la mesa disponible `0,838 × 0,84 × 0,77 m` pasa el gate dimensional de shadow; 20/44 candidatos del OBB simplificado son alertas contra proxies conservadores, no contactos demostrados ni descarte. Sin robot/red/ROS/contenedor/publicador/movimiento. E6.1=40 %, plan ponderado=45,8 %, tasks físicas=0/4; faltan fixture/ENTRY vivos y 5 shadow |
