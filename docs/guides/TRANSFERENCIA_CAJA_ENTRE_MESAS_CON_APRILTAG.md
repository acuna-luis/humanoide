...
| Estación | AprilTag | Necesidad | Función |
| --- | ---: | --- | --- |
| Mesa 1 | 112 | Opcional | Referencia de origen y recuperación futura |
| Mesa 2 | 113 | Recomendada | Alineación fina para el depósito |
...
validación integrada está desactivada mediante `use_apriltag_val: false`. Para
esta guía se utiliza el detector AprilTag independiente.
...
ros2 service call /apriltag/start_detecting \
  ...
  "{start_detecting: true, img_topic_name: '/sensor/camera/stereo/color/raw', tag_id: 113, tag_size: 0.10, tag_frame: 'mesa2_tag113'}"
```
...
ros2 service call /apriltag/start_detecting \
  sensor_task_msgs/srv/AprilTagStartDetecting \
  "{start_detecting: false, img_topic_name: '/sensor/camera/stereo/color/raw', tag_id: 113, tag_size: 0.10, tag_frame: 'mesa2_tag113'}"
```
...
Debe crearse un orquestador, por ejemplo
`scripts/cruzr_blue_workbin_table_transfer.sh`, que implemente:
...
Los componentes existentes que puede reutilizar son:
...
Estas órdenes no forman todavía por sí solas un ciclo entre mesas: faltan la
navegación específica hasta `MESA2_PRE`, la alineación AprilTag y la gestión de
errores entre etapas.

## 9. Validación progresiva

La puesta en servicio debe realizarse en este orden:

1. Detectar el tag con el robot inmóvil.
2. Navegar sin caja hasta `MESA2_PRE`.
3. Ejecutar la alineación local sin caja y sin mover los brazos.
4. Repetir la alineación desde varias poses iniciales.
5. Transportar la caja hasta `MESA2_PRE` y detenerse manteniéndola sujeta.
6. Validar que el tag continúa visible con la caja agarrada.
7. Ejecutar el depósito completo con la caja vacía.
8. Repetir el ciclo completo únicamente después de obtener resultados
   consistentes.
