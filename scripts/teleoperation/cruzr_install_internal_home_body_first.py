...
XML = ROOT/'scripts/teleoperation/tasks/cruzr_internal_home_body_first_v4_20s.xml'
...
NEW_SHA = 'e3d0656424a3611d89262ae645f127d975920fd09c437f9ef9c07725d69dc49c'
...
def status(evidence):
    ...
    print('HOME_DEFINITION='+('body-first-v4-20s' if sha == NEW_SHA else 'open-v3-20s'))
    return sha
...
backup=pathlib.Path('/etc/walker/trajectory-overlays')/(datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')+'_home_body_first_v4')
backup.mkdir(parents=True,exist_ok=False)
shutil.copy2(target,backup/'home.before.xml')
info=target.stat()
fd,temp=tempfile.mkstemp(prefix='.home.body-first-v4.',dir=target.parent)
try:
...
manifest={'target':str(target),'before_sha256':expected,'after_sha256':new_sha,
 ...
 'note':'Physical trial pending. Original backup restores open-v3 HOME; no automatic rollback.'}
...
def main():
    ...
    if not (args.preflight or args.install or args.reload):
        ...
        print('LOCAL_CHECK_OK=home-body-first-v4-20s; MOVEMENT_COMMANDS=0; SIX_SECOND_DRAFT=not-installable')
        return
...
'''
    ...
    (evidence/'cruzr_internal_home_body_first_v4_20s.xml').write_bytes(data)
